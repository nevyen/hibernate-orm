...
Hibernate Spatial supports two different geometry models: http://docs.geotools.org/latest/userguide/library/jts/index.html[JTS] and
...
You need to include the `hibernate-spatial` dependency in your build environment. Further you need the dependecies for GeoLatte.
...
So, we can use the `PostgisDialect` but we should use the dialect which matches our database the best.
...
[[spatial-configuration-wildfly]]
=== WildFly

There seems to be classpath problems in a J2EE container where the JDBC drivers live in a different classpath than the Postgis JDBC extensions and/or Hibernate Spatial.

So to get them fixed you need to copy the jars from Hibernate Spatial, JTS and GeoLatte to `JBOSS_PATH/modules/system/layers/base/org/hibernate/main`.
Than you need to add them to the `module.xml`, so in the `resource` tag add the following
====
[source,xml]
----
<resource-root path="hibernate-spatial-x.x.x.jar"/>
<resource-root path="jts-x.xx.jar"/>
<resource-root path="geolatte-geom-x.x.x.jar"/>
----
====
...
----
====
If you are using postgresql, you need to add in the dependecies tag: 
====
[source,xml]
----
